...
export const buildSchema = async () => {
  ...
  const format = await promptList('Select the output format:', [
  ...
  const filePath = path.join(outDir, `${componentName.toLowerCase()}${formatExt(format)}`)
...
function generateSchemaContent (schema, format, nameVar) {
  ...
  switch (format) {
    ...
    case 'ts':
      ...
      return ('import type { Schema } from "req-valid-express";\n\n' +

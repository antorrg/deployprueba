...
export async function promptConfirm (message, options) {
  ...
  return new Promise((resolve) => {
    rl.question(query, (answer) => {
      ...
      if (val === 'y' || val === 'yes') { resolve(true) } else if (val === 'n' || val === 'no') { resolve(false) } else { resolve(options?.default ?? false) }
    })
  })
}
export async function promptList (message, choices) {
  const parsedChoices = choices.map((c) => typeof c === 'object' && c !== null && 'value' in c
    ? c

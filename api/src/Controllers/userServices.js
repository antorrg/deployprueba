import { User, Car } from '../Configs/database.js'
...
export default {
  ...
  userLogin: async (email, password) => {
    try {
      ...
      if (!userfound) { { throwError('Usuario no registrado', 400) } } else if (userfound.enable === false) { throwError('Usuario bloqueado', 400) } else {
        const passwordMatch = await bcrypt.compare(password, userfound.password)
        if (!passwordMatch) { throwError('Email o password no validos', 400) }
        return {
          data: parsedUser(userfound),
